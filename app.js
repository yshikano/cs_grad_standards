...
function buildDatasetCandidates() {
  ...
  pushCandidate('./data/courses.sample.json', '相対パス ./data/courses.sample.json');
  pushCandidate('data/courses.sample.json', '相対パス data/courses.sample.json');
  pushCandidate(new URL('./data/courses.sample.json', document.baseURI).href, 'document.baseURI 基準');

  const appScript = Array.from(document.scripts).find(script => /(^|\/)app\.js(\?|$)/.test(script.src || ''));
  if (appScript?.src) {
    pushCandidate(new URL('./data/courses.sample.json', appScript.src).href, 'app.js の場所基準');
  }
